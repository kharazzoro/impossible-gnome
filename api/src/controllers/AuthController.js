...
class AuthController extends Controller {
  constructor() {
    ...
    this.plugin("hapi-auth-cookie", (err, server) => {
      //this code configures the auth strategy for all requests
  ...
  recover(request, reply) {
    ...
    userModel
      ...
      .then((accept, reject, authUser) => {
        if (authUser.fromFacebook && !authUser.password) {
          reply({
            msg: "Facebook users cannot recover passwords through this service"
          }).code(422);
        } else {
  ...
  inviteUsers(request, reply) {
    let Emails;
    try {
      Emails = JSON.parse(request.payload.emails).filter(
            ...
            userModel
              ...
              .done(user => {
                if (user) {
                  ...
                  profileModel
                    .followProfile(request.auth.credentials.userID, user.userID)
                    .done(accept)
                    .error(reject);
                } else {
                  userModel
